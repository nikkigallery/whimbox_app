...
const API = {
  ...
  videoOverlay: {
    setIgnoreMouseEvents: (ignore: boolean, options?: { forward?: boolean }) =>
      ipcRenderer.invoke('video-overlay:set-ignore-mouse-events', ignore, options),
  ...
  mapMaskOverlay: {
    ...
    getBounds: () =>
      ipcRenderer.invoke('map-mask-overlay:get-bounds') as Promise<{
        x: number
        y: number
        width: number
        height: number
    ...
    syncToGameWindow: () =>
      ipcRenderer.invoke('map-mask-overlay:sync-to-game-window') as Promise<{
        x: number
        y: number
        width: number
        height: number
